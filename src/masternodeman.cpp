...
void CMasternodeMan::LocalDiagnostic(int nBlockHeight, int& nQSTEESNODE_1Ret, int& nQSTEESNODE_5Ret, int& nQSTEESNODE_10Ret)
{
    ...
    for (auto& mnpair : mapMasternodes) {
        ...
        if (mnpair.second.GetSinType() == CMasternode::SinType::QSTEESNODE_1) {
            nQSTEESNODE_1Ret = 1;
        }
        
        //found QSTEESNODE_5 in network
        if (mnpair.second.GetSinType() == CMasternode::SinType::QSTEESNODE_5) {
            nQSTEESNODE_5Ret = 1;
        }
        
        //found QSTEESNODE_10 in network
        if (mnpair.second.GetSinType() == CMasternode::SinType::QSTEESNODE_10) {
            nQSTEESNODE_10Ret = 1;
        }
    }
}
...
bool CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCountRet, masternode_info_t& mnInfoRet, CMasternode::SinType vSinType)
{
    ...
    if ( vSinType == CMasternode::SinType::QSTEESNODE_UNKNOWN ) { return false; }
